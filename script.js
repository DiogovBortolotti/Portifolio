...
buttonGeral.forEach((item) => {
    item.addEventListener('click', (e) => {
        ...
        if (currentButton.classList.contains('all')) {
            showLista(listaALL);
        } if (currentButton.classList.contains('documentacao')) {
            showLista(listaALL, "documentacao")
        }

        if (currentButton.classList.contains('back')) {
            showLista(listaALL, "back")
        }
...
function escreverPalavra() {
    elementoTexto.textContent = "ㅤ";
    let i = 0;
    const textoAtual = palavras[palavraAtual];
    const intervalo = setInterval(() => {
        elementoTexto.textContent += textoAtual[i];
        i++;
        if (i === textoAtual.length) {
            clearInterval(intervalo);
            setTimeout(apagarTexto, 1000); // Aguarda 1 segundo antes de apagar o texto
        }
    }, 100);
}
...
function apagarTexto() {
    const intervalo = setInterval(() => {
        ...
        if (textoAtual.length === 1) { // Verifica se há apenas o asterisco restante
            clearInterval(intervalo);
            palavraAtual = (palavraAtual + 1) % palavras.length; // Próxima palavra ou volta para a primeira
            setTimeout(escreverPalavra, 500); // Aguarda 0.5 segundo antes de começar a escrever a próxima palavra
        } else {
...
escreverPalavra(); // Inicializa o efeito com a primeira palavra
